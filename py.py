...
def generate_lisp(code):
    if isinstance(code, ast.Module):
        ...
        if vars:
            vars_str = ' '.join(f'({v} 0)' for v in vars)
            body = f'(let ({vars_str})\n{indented(body)})'
        ...
        return f'(return {generate_lisp(code.value)})'
    ...
    elif isinstance(code, ast.AugAssign):
        ...
        raise ValueError(f'Unexpected node types: {ast.dump(code, indent=4)}')
