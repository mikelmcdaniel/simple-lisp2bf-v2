...
def asm_line_to_bf(asm_line):
	...
	if inst == 'jump':
		if len(asm_line) > 1:
			return '%s ]' % (asm_line_to_bf(normalized_asm_line(
				'push %s' % (asm_line[1],))),)
		else:
			return ']'
	elif inst == 'push':
		...
		return '%s %s' % tuple(asm_line_to_bf(normalized_asm_line(x)) for x in (
			'sub', 'not'))
	elif inst == 'lesseq':
		return '<[-<-[->>>+>+<<<<]>>>>[-<<<<+>>>>]+<[->-<[-]]>[-<+>]' + \
			'<[<+<[-]>>-]<<]<[-]>>[-<<+>>]<'
		...
		return '%s %s %s' % tuple(asm_line_to_bf(normalized_asm_line(x)) for x in (
			'copy -2', 'pop -3', 'lesseq'))
	elif inst == 'sub':
		return '<[-<->]'
	elif inst == 'not':
		...
		label = inst[:-1]
		return '%s %s %s %s <[- %s' % tuple(asm_line_to_bf(normalized_asm_line(x))
			for x in ('copy', 'push %s' % (label,), 'sub', 'not', 'pop'))
	elif inst == 'cond':
		return '%s %s %s %s %s %s %s' % tuple(asm_line_to_bf(normalized_asm_line(x))
...
if __name__ == '__main__':
	...
	#eval_asm(in_file.read(), stdin, out_file)
